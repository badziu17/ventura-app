...
{id:"u2",name:"Emilia",email:"emilia@ventura.app",color:C.coral,role:"user",photoUrl:"",social:{instagram:"",facebook:"",tiktok:"",youtube:""},friends:["u1"],prefs:{currency:"PLN",travelStyle:"Foodie",homeAirport:"GDN"}},
...
const mkItems=(arr)=>arr.map((a,i)=>({...a,id:a.id||"i"+i,votes:{u1:1,u2:1},fav:{},status:"approved",ticketUrl:a.ticketUrl||"",ticketBought:a.ticketBought||false}));
const INIT_TRIPS=[
...
dayData:[
{day:1,date:"Sat, Mar 7",title:"Historic Center Walk",img:"https://images.unsplash.com/photo-1585407925232-33158f7be498?w=600&h=200&fit=crop&q=80",weather:{hi:12,lo:4,icon:"sun"},steps:{u1:18420,u2:17850},items:mkItems([{time:"09:00",name:"Ogrody Cismigiu",desc:"Oldest park",type:"sight",duration:"1h",cost:0,rating:4.5},{time:"10:15",name:"Palatul Kretzulescu",type:"sight",duration:"30min",cost:0,rating:4.3},{time:"12:00",name:"Caru' cu Bere",desc:"1879 beer hall",type:"food",duration:"1.5h",cost:120,rating:4.4},{time:"14:00",name:"National Art Museum",type:"museum",duration:"2h",cost:30,rating:4.5,ticketUrl:"https://www.mfrp.ro/bilete",ticketBought:true},{time:"16:30",name:"Ateneul Roman",desc:"1888 concert hall",type:"sight",duration:"45min",cost:25,rating:4.8,ticketUrl:"",ticketBought:false},{time:"20:15",name:"Grand Cafe Van Gogh",type:"food",duration:"1.5h",cost:180,rating:4.3}])},
{day:2,date:"Sun, Mar 8",title:"Transylvania Castles",img:"https://images.unsplash.com/photo-1596379448498-e498b9e7ba0b?w=600&h=200&fit=crop&q=80",weather:{hi:9,lo:1,icon:"cloud"},steps:{u1:22100,u2:21800},items:mkItems([{time:"07:00",name:"Depart to Sinaia",type:"transport",duration:"2h",cost:0},{time:"09:30",name:"Castelul Peles",desc:"Neo-Renaissance palace",type:"sight",duration:"2h",cost:80,rating:4.8,ticketUrl:"https://pyvara.ro/bilet-peles",ticketBought:true},{time:"12:00",name:"Lunch in Sinaia",type:"food",duration:"1h",cost:90,rating:4.2},{time:"13:30",name:"Castelul Bran",desc:"Dracula's castle",type:"sight",duration:"1.5h",cost:60,rating:4.3,ticketUrl:"https://www.bfrp.ro/bilete-bran",ticketBought:true},{time:"15:30",name:"Brasov Old Town",type:"sight",duration:"2h",cost:15,rating:4.6},{time:"18:00",name:"Return to Bucharest",type:"transport",duration:"2.5h",cost:0}])},
{day:3,date:"Mon, Mar 9",title:"Parliament & Farewell",img:"https://images.unsplash.com/photo-1587974928442-77dc3e0dba72?w=600&h=200&fit=crop&q=80",weather:{hi:14,lo:5,icon:"partlysunny"},steps:{u1:14200,u2:13900},items:mkItems([{time:"10:00",name:"Palatul Parlamentului",desc:"2nd largest building",type:"sight",duration:"2h",cost:50,rating:4.6,ticketUrl:"https://cfrp.ro/bilete-parlament",ticketBought:false},{time:"13:15",name:"Lacrimi si Sfinti",type:"food",duration:"1.5h",cost:150,rating:4.6},{time:"15:00",name:"Calea Victoriei",type:"sight",duration:"1h",cost:25,rating:4.5},{time:"17:30",name:"Bolt to Airport",type:"transport",duration:"45min",cost:60}])}],
...
packing:{"Clothing":[{item:"Warm layers",qty:3,packed:true,assignedTo:"u1"},{item:"Winter jacket",qty:1,packed:true,assignedTo:"u1"},{item:"Walking shoes",qty:1,packed:true,assignedTo:"u2"},{item:"Scarf & gloves",qty:1,packed:false,assignedTo:"u2"}],"Electronics":[{item:"Phone chargers",qty:2,packed:true,assignedTo:null},{item:"Power bank",qty:1,packed:true,assignedTo:"u1"},{item:"EU adapter",qty:1,packed:false,assignedTo:null}],"Documents":[{item:"ID cards",qty:2,packed:false,assignedTo:null},{item:"Booking confirmations",qty:1,packed:false,assignedTo:"u1"}]},
...
const[ticketEdit,setTE]=useState(null);
const[ticketUrl,setTU]=useState("");
...
// Get items needing tickets from all days
const getTicketItems=tr=>{if(!tr?.dayData)return[];const items=[];tr.dayData.forEach(day=>{day.items.forEach(it=>{if(it.cost>0&&it.type!=="food"&&it.type!=="transport"){items.push({...it,dayNum:day.day,dayTitle:day.title})}})});return items};
...
const createTrip=()=>{if(isGuest){alert(t("loginToSave"));return}const id="trip_"+Date.now();setTrips(p=>[...p,{id,name:wData.name||`${wData.dest} Trip`,dest:wData.dest,dates:`${wData.startDate} – ${wData.endDate}`,travelers:[currentUser.id],observers:[],days:0,status:"planning",currency:"PLN",vacStatus:{[currentUser.id]:"not_applied"},heroImg:"",budget:{total:0},completeness:5,journal:[],memories:null,dayData:[],expenses:[],packing:{},deals:[],comparisons:[],planningTips:[{icon:"🏨",title:{pl:"Nocleg",en:"Accommodation"},desc:{pl:`Znajdź w ${wData.dest}.`,en:`Find in ${wData.dest}.`},done:false,priority:"high"},{icon:"✈️",title:{pl:"Loty",en:"Flights"},desc:{pl:"Szukaj ofert.",en:"Search deals."},done:false,priority:"high"},{icon:"🗺️",title:{pl:"Plan dnia",en:"Itinerary"},desc:{pl:`${wData.travelers} osób.`,en:`${wData.travelers} people.`},done:false,priority:"medium"},{icon:"🎒",title:{pl:"Lista pakowania",en:"Packing list"},desc:{pl:"Szablon lub od zera.",en:"Template or scratch."},done:false,priority:"low"}]}]);setAT(id);setScr("trip");setTab("plan");setSW(false);setWS(0);setWD({name:"",dest:"",startDate:"",endDate:"",travelers:2,style:"Cultural"})};
...
:<span style={{fontSize:11,fontWeight:600,color:sC[vs],padding:"3px 8px",borderRadius:6,background:`${sC[vs]}15`}}>{sL[vs]}</span>}</div>)})}
{allVacApproved&&<div style={{marginTop:10,padding:16,borderRadius:14,background:`linear-gradient(135deg,${C.sageLight},#e8f5e9,${C.blueLight})`,border:`1.5px solid ${C.sage}44`,textAlign:"center",animation:"fadeUp 0.4s"}}><div style={{fontSize:36,marginBottom:6}}>🎉</div><div style={{fontSize:15,fontWeight:800,fontFamily:Fn,color:C.sage,marginBottom:4}}>{t("allApprovedTitle")}</div><p style={{fontSize:12,color:C.textSec,lineHeight:1.5,marginBottom:10}}>{t("allApprovedMsg")}</p><button onClick={()=>setTab("plan")} style={{padding:"10px 24px",borderRadius:99,background:`linear-gradient(135deg,${C.sage},${C.blue})`,border:"none",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:C.shadow}}>{t("letsGo")}</button></div>}
</Card>}
...
{it.assignedTo&&!canE&&<span style={{fontSize:10,padding:"2px 6px",borderRadius:4,background:getU(it.assignedTo).color+"20",color:getU(it.assignedTo).color,fontWeight:600}}>{getU(it.assignedTo).name}</span>}
...
<div style={{display:"flex",gap:6}}><button onClick={()=>{if(!newExp.name||!newExp.amount)return;upT(trip.id,tr=>({...tr,expenses:[...tr.expenses,{id:Date.now(),name:newExp.name,cat:newExp.cat,amount:parseFloat(newExp.amount),currency:newExp.currency,payer:currentUser.id,date:lang==="pl"?"Dziś":"Today"}]}));setNE({name:"",cat:"Food",amount:"",currency:cur});setNEO(false)}} style={{flex:2,padding:"10px",borderRadius:8,background:C.sage,border:"none",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{t("save")}</button>
...
{/* Tickets Required */}
{(()=>{const allItems=(trip.dayData||[]).flatMap((d,dI)=>d.items.filter(it=>it.cost>0&&it.type!=="food"&&it.type!=="transport").map(it=>({...it,dayIdx:dI,dayTitle:`${lang==="pl"?"Dzień":"Day"} ${d.day}: ${d.title}`})));const needTickets=allItems.filter(it=>it.ticketUrl!==""||it.cost>=20);return needTickets.length>0&&<><div style={{fontSize:14,fontWeight:700,fontFamily:Fn,marginTop:4,marginBottom:6}}>🎟️ {t("ticketsNeeded")}</div><p style={{fontSize:12,color:C.textSec,marginBottom:10}}>{t("ticketsDesc")}</p>{needTickets.map((it,i)=>(<Card key={it.id||i} style={{padding:14,marginBottom:8,border:`1px solid ${it.ticketBought?C.sage:C.gold}33`,background:it.ticketBought?C.sageLight:C.goldLight}} hover={false}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:16}}>{tE[it.type]||"📍"}</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{it.name}</div><div style={{fontSize:11,color:C.textDim}}>{it.dayTitle} · {it.time} · {it.cost} {trip.currency}</div>{it.ticketUrl&&<a href={it.ticketUrl} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:C.blue,textDecoration:"none"}}>🔗 {lang==="pl"?"Link do biletu":"Ticket link"} ↗</a>}</div><span style={{fontSize:10,fontWeight:700,padding:"4px 10px",borderRadius:99,background:it.ticketBought?C.sage:C.gold,color:"#fff"}}>{it.ticketBought?t("ticketBought"):t("ticketNeeded")}</span></div>{canE&&<div style={{display:"flex",gap:6,marginTop:8}}>{!it.ticketBought&&<button onClick={()=>upT(trip.id,tr=>({...tr,dayData:tr.dayData.map((d,di)=>di===it.dayIdx?{...d,items:d.items.map(x=>x.id===it.id?{...x,ticketBought:true}:x)}:d)}))} style={{padding:"5px 12px",borderRadius:6,background:C.sage,border:"none",color:"#fff",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>✓ {lang==="pl"?"Oznacz jako kupiony":"Mark as bought"}</button>}{<button onClick={()=>{const url=prompt(t("addTicketLink"),it.ticketUrl||"https://");if(url!==null)upT(trip.id,tr=>({...tr,dayData:tr.dayData.map((d,di)=>di===it.dayIdx?{...d,items:d.items.map(x=>x.id===it.id?{...x,ticketUrl:url}:x)}:d)}))}} style={{padding:"5px 12px",borderRadius:6,background:C.bgAlt,border:`1px solid ${C.border}`,color:C.textSec,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>🔗 {t("addTicketLink")}</button>}</div>}</Card>))}</>})()}
